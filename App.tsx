...
const App: React.FC = () => {
  ...
  const handleRemoveSlot = (indexToRemove: number) => {
    const remaining = inputSlots.filter((_, idx) => idx !== indexToRemove);
    const reindexed = remaining.map((slot, idx) => ({ ...slot, id: `A${idx + 1}` }));
    setInputSlots(reindexed);
  };

  const handleAddSlot = () => {
    setInputSlots([...inputSlots, { id: `A${inputSlots.length + 1}`, type: 'file', files: [], pathDisplay: '' }]);
  };
  ...
  const handleStartProcess = async (isSelfTest = false) => {
    ...
    if (isSelfTest) {
      ...
      inputSlots.forEach(slot => {
        slot.files.forEach(f => {
          if (!f.name.toLowerCase().endsWith('.png') && !f.name.toLowerCase().endsWith('.jpg') && !/\.(ttf|otf|woff2?)$/i.test(f.name)) {
            ...
            if (rel) {
               ...
               if (parts.length > 1) finalPath = `${cleanBasePath}/${parts.slice(1).join('/')}`;
               else finalPath = cleanBasePath;
            }
    ...
    try {
      setLog(prev => [...prev, "[SYNC] Compiling archive...", "downloading"]);
  ...
  const handleOcrProcess = async () => {
    ...
    if (!navigator.onLine) {
       setLog(prev => [...prev, "[ERR] Offline. AI Unavailable.", "[INFO] Use manual mapping in Settings > Typography."]);
       setIsFinished(true);
       return;
    }
    ...
    for (const fileObj of (files as File[])) {
      ...
      } catch { setLog(prev => [...prev, `[ERR] Scan failed for ${f.name}`]); }
    }
    ...
    for (const file of files) {
      ...
      if (['ttf', 'otf', 'woff', 'woff2'].includes(ext || '')) {
        try {
          ...
          newFonts.push({ id: `${file.name}-${Date.now()}`, name: file.name.replace(/\.[^/.]+$/, ""), fileName: file.name, data: base64, format: ext as any });
        } catch (e) { console.error("Font load error", e); }
      }
    }
    if (newFonts.length > 0) setCustomFonts(prev => [...prev, ...newFonts]);
  };

  const handleRemoveFont = (id: string) => setCustomFonts(prev => prev.filter(f => f.id !== id));

  const handleTerminalConfirm = (path: string) => {
    if (terminalTargetSlot !== null) {
      const ns = [...inputSlots];
      ns[terminalTargetSlot].customPath = path;
      ns[terminalTargetSlot].pathDisplay = path.endsWith('/') ? path : `${path}/`;
      setInputSlots(ns);
  ...
  return (
    <div className={`min-h-screen ${t.root} flex flex-col overflow-y-auto custom-scrollbar transition-colors duration-500`} style={t.style}>
      ...
      {view === 'paths' && <PathsView 
        slots={inputSlots} activeMode={'MIXED_MODE'} 
        onBack={() => {}} onNext={() => setView('config')} onOpenSettings={() => setView('settings')} 
        ...
        onRunDiagnostic={() => handleStartProcess(true)} 
        t={t} />}
      ...
      {isInspectorOpen && <InspectorModal slots={inputSlots} folderSortCriteria={folderSortCriteria} folderSortDesc={folderSortDesc} fileSortCriteria={fileSortCriteria} fileSortDesc={fileSortDesc} onClose={() => setIsInspectorOpen(false)} setFolderSortCriteria={setFolderSortCriteria} setFolderSortDesc={setFolderSortDesc} setFileSortCriteria={setFileSortCriteria} setFileSortDesc={setFileSortDesc} t={t} />}
      ...
      <TerminalPathModal isOpen={terminalTargetSlot !== null} currentId={terminalTargetSlot !== null ? inputSlots[terminalTargetSlot].id : ''} onClose={() => setTerminalTargetSlot(null)} onConfirm={handleTerminalConfirm} />

      <input type="file" ref={fileInputRef} className="hidden" multiple accept="*" onChange={(e) => {
        const target = e.target as HTMLInputElement; const files = target.files; const idx = activeSlotIndex.current; if (!files || idx === null) return;
