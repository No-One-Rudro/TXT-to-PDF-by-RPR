...
import { ArrowLeft, Type, FileType, Image as ImageIcon, Sparkles, Activity, Search, Database, AlertTriangle, Plus, Trash2, Edit2, Check } from 'lucide-react';
...
const FontsView: React.FC<Props> = ({ onBack, fonts, onRemoveFont, onAddFonts, t }) => {
  ...
  // Fix: Explicitly type registryEntries to avoid 'unknown' inference in some environments
  ...
  return (
    <div className="max-w-2xl mx-auto w-full p-8 flex-1 flex flex-col animate-in fade-in duration-500 pb-48">
      ...
      <div className="bg-zinc-950/30 border border-white/5 rounded-[3.5rem] p-10 mb-10 backdrop-blur-xl relative overflow-hidden group">
          ...
          <div className="flex justify-between items-start">
            <div className="flex items-center space-x-4">
              ...
              <div>
                ...
                <p className="text-[9px] font-bold uppercase tracking-widest text-zinc-500 mt-1">Learned characters across sessions</p>
              </div>
            </div>
      ...
      <div className="space-y-12">
        ...
        <section className="space-y-6">
          <h3 className="text-[10px] font-black uppercase tracking-[0.5em] text-zinc-500 px-6">Missing Character Log</h3>
          <div className="space-y-3">
            {missing.map(char => (
              <div key={char} className="bg-red-500/5 border border-red-500/10 p-5 rounded-3xl flex justify-between items-center group">
                <div className="flex items-center space-x-4">
                  <AlertTriangle size={16} className="text-red-500/40" />
                  <span className="font-mono text-xs font-black text-red-500/60 uppercase">{char}</span>
                </div>
                <button className="text-[10px] font-black uppercase text-green-accent opacity-0 group-hover:opacity-100 transition-all flex items-center space-x-2">
                   <Edit2 size={12}/> <span>Link Asset</span>
                </button>
              </div>
            ))}
            {missing.length === 0 && (
              <p className="text-center text-[10px] font-black uppercase tracking-widest opacity-20">No Missing Character Telemetry</p>
            )}
          </div>
        </section>
